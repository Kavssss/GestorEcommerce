...
//		}
	...
	private void inserirOutrosItens() {
		switch (linhasVisiveis) {
		case 2:
			String codItem = cbItem2.getSelectionModel().getSelectedItem();
			Integer qtde = !txtQtde2.getText().isBlank() ? Integer.parseInt(txtQtde2.getText()) : null;
			Double valorUnitario = !txtValorUnitario2.getText().isBlank() ? Double.valueOf(txtValorUnitario2.getText()) : null;
			Double valorTotal = CalculaTotalERecebido.calculaTotal(qtde, valorUnitario);
			...
			break;
		case 3:
			codItem = cbItem3.getSelectionModel().getSelectedItem();
			qtde = !txtQtde3.getText().isBlank() ? Integer.parseInt(txtQtde3.getText()) : null;
			valorUnitario = !txtValorUnitario3.getText().isBlank() ? Double.valueOf(txtValorUnitario3.getText()) : null;
			valorTotal = CalculaTotalERecebido.calculaTotal(qtde, valorUnitario);
			valorRecebido = CalculaTotalERecebido.calculaRecebidoShopee(valorTotal, qtde);
			try {
				shopeeController.insertItemVenda(codItem, qtde, valorUnitario, valorTotal, valorRecebido);
			} catch (SQLException e) {
				Alerts.showAlert("SQL Exception", "ERRO", e.getMessage(), AlertType.ERROR);
			...
			break;
		case 4:
			codItem = cbItem4.getSelectionModel().getSelectedItem();
			qtde = !txtQtde4.getText().isBlank() ? Integer.parseInt(txtQtde4.getText()) : null;
			valorUnitario = !txtValorUnitario4.getText().isBlank() ? Double.valueOf(txtValorUnitario4.getText()) : null;
			valorTotal = CalculaTotalERecebido.calculaTotal(qtde, valorUnitario);
			valorRecebido = CalculaTotalERecebido.calculaRecebidoShopee(valorTotal, qtde);
			try {
				shopeeController.insertItemVenda(codItem, qtde, valorUnitario, valorTotal, valorRecebido);
			...
			break;
		case 5:
			codItem = cbItem5.getSelectionModel().getSelectedItem();
			qtde = !txtQtde5.getText().isBlank() ? Integer.parseInt(txtQtde5.getText()) : null;
			valorUnitario = !txtValorUnitario5.getText().isBlank() ? Double.valueOf(txtValorUnitario5.getText()) : null;
			valorTotal = CalculaTotalERecebido.calculaTotal(qtde, valorUnitario);
			valorRecebido = CalculaTotalERecebido.calculaRecebidoShopee(valorTotal, qtde);
			try {
				shopeeController.insertItemVenda(codItem, qtde, valorUnitario, valorTotal, valorRecebido);
